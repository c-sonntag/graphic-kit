...
namespace gk {
  namespace math {
    ...
    template<typename T>
    __forceinline bool is_pow_of( T base, T number )
    {
    ...
    __forceinline in_circle_property::in_circle_property( const float x, const float y, const float x_center, const float y_center ) :
    ...
    __forceinline void dist_xyz( glm::vec3& position, const float decal )
    {
    ...
    __forceinline void rot_xz( glm::vec3& position, const float decal )
    {
    ...
    __forceinline void rot_zy( glm::vec3& position, const float decal )
    {
    ...
    __forceinline void rot_y( glm::vec3& position, const float decal )
    {
    ...
    __forceinline uint negative_mod( const int pos, const int m )
    { return static_cast<uint>( pos % m + ( pos % m < 0 ? m : 0 ) ); }
