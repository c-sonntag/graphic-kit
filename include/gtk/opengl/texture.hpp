  ...
  namespace opengl {
    ...
    inline raiigl::texture texture_from_image( const encoder::image & img )
    {
      ...
      const bool good_image_format(
        gtk::math::is_pow_of( 2u, img.width ) &&
        gtk::math::is_pow_of( 2u, img.height )
      );
      if ( !good_image_format )
      {
        std::ostringstream oss;
        oss << "[gtk::opengl::texture_from_image] Incompatible image dimension ("
      ...
      else if ( img.channels == 4 )
      {
        ...
        throw std::runtime_error(
          "[gtk::opengl::texture_from_image] Unknown channels("
