...
namespace gtools {
  namespace opengl {
    ...
    inline raiigl::shader quick_shader::open_from_file_path( const std::string & shader_path, const raiigl::shader_type type )
    {
      ...
      //
      if ( !shader_ifs )
        throw std::runtime_error( "[gtools::opengl::quick_shader] Can't open shader file  : " + shader_path );
    ...
    __forceinline raiigl::shader quick_shader::open_from_local_erc( const erc::file_id & shader_erc_id, const raiigl::shader_type type )
    {
      ...
      //
      if ( !shader_erc_p )
        throw std::runtime_error( "[gtools::opengl::quick_shader] Can't open embedded resource file : \"" + shader_erc_id.package_name + ":" + shader_erc_id.file_path + "\"" );
